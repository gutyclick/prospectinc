alter table public.searches
  ...
  add column processing_stage text not null default 'pendiente'
    check (processing_stage in ('pendiente', 'descubriendo', 'guardando', 'preparando', 'finalizado', 'fallido')),
  add column retry_count integer not null default 0 check (retry_count >= 0);

create unique index searches_one_active_fingerprint_idx
  on public.searches (owner_id, query_fingerprint)
  where query_fingerprint is not null and status in ('pendiente', 'analizando');

create function public.persist_discovery_results_for_owner(
...
begin
  ...
  loop
    ...
    select id into target_id from public.prospects
      where owner_id = expected_owner_id and google_place_id = business ->> 'placeId';

    if target_id is null then
      insert into public.prospects (
        owner_id, search_id, google_place_id, business_name, niche, primary_type,
        formatted_address, city, country, latitude, longitude, website_url,
        google_maps_url, rating, reviews_count, website_status, opportunity_score,
        commercial_status, recommended_offer, ai_summary, detected_opportunities
      ) values (
        expected_owner_id, search_record_id, business ->> 'placeId', business ->> 'displayName',
        current_search.query, nullif(business ->> 'primaryType', ''),
        nullif(business ->> 'formattedAddress', ''), current_search.location,
        current_search.country, nullif(business ->> 'latitude', '')::double precision,
        ...
        case when nullif(business ->> 'websiteUrl', '') is null then 'sin-sitio'::public.website_status else 'desconocido'::public.website_status end,
        score, 'nuevo',
      ...
      update public.prospects set
        search_id = search_record_id, business_name = business ->> 'displayName',
    ...
    if phone is not null and normalized_phone <> '' then
      insert into public.contact_points (
        ...
        expected_owner_id, target_id, 'phone', phone, normalized_phone,
        business ->> 'sourceUrl', true, now(), now()
      ) on conflict (prospect_id, type, normalized_value)
        do update set value = excluded.value, source_url = excluded.source_url, last_verified_at = now();
    end if;
  end loop;

  update public.searches set
    results_count = jsonb_array_length(discovered), opportunities_count = opportunities,
    inserted_count = inserted, deduplicated_count = deduplicated,
    provider_call_count = provider_call_count + 1
  where id = search_record_id and owner_id = expected_owner_id;

  insert into public.activities (owner_id, type, description, metadata)
  values (
    expected_owner_id, 'busqueda',
    'Google Places devolvió ' || jsonb_array_length(discovered) || ' negocios.',
...
revoke all on function public.persist_discovery_results_for_owner(uuid, uuid, jsonb) from public, anon, authenticated;
grant execute on function public.persist_discovery_results_for_owner(uuid, uuid, jsonb) to service_role;
